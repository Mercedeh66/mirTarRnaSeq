...
The example data set for the first approach is 25 matching miRNA and mRNA EBV positive samples from TCGA. We attempt to identify the EBV miRNA targets on EBV genome (part1). 

The second example set set is the mouse fibroblast differntiated to muscle cells in three time points. Here, we try to identify mRNA targets of miRNA experssed at various time points (parts 2 and 3).
...
```{r gauss, eval=TRUE, echo=TRUE,out.width="60%", fig.align="center"}
...
G <- do.call(rbind.data.frame, blaGaus[["AICvalues"]]) #To test AIC model performance
names(G) <- c("AIC_G")
plot(density(G$AIC_G)) #Low values seems like a reasonable model
GM <- melt(G)
```

## Running poisson model over all individual miRNA mRNA models

```{r poisson, eval=TRUE, echo=TRUE, out.width="60%", fig.align="center"}
...
```{r negbin, eval=TRUE, echo=TRUE, out.width="60%", fig.align="center"}
...
```{r zeroinfnegbin, eval=TRUE, echo=TRUE, out.width="60%", fig.align="center"}
...
```{r zeroinfpoisson, message=FALSE, echo=TRUE, cache=FALSE, results=TRUE, out.width="60%", fig.align="center"}
...
Combine <- combiner(DiffExp, miRNAExp, miRNA_select)
geneVariant <- geneVari(Combine, miRNA_select)
MultiModel <- runModels(Combine, geneVariant, miRNA_select, family = glm_multi(), mode="multi", scale = 10)
...
Combine <- combiner(DiffExp, miRNAExp, miRNA_select)
geneVariant <- geneVari(Combine, miRNA_select)
InterModel <- runModels(Combine, geneVariant, miRNA_select,
...
All_miRNAs_run<-runAllMirnaModels(mirnas =vMiRNA[1:5] , DiffExpmRNA = DiffExpmRNASub, DiffExpmiRNA = miRNAExp,
